...
public class Helper {
	...
	/**
	 * Get the total shutdown time (total period) of 
	 * @param timeBeforeHostShutdown
	 * @return
	 * @author minxianx
	 */
	public static double getTotalShutdownTime(List<Double> timeBeforeHostShutdown){
		double totalShutDownTime = 0.0;
		for(Double shutdownTime: timeBeforeHostShutdown){
			totalShutDownTime += shutdownTime.doubleValue();
		}
	...
	public static void printResults(
		...
		double totalShutdownTime = getTotalShutdownTime(timeBeforeHostShutdown);

		int numberOfHostShutdowns = timeBeforeHostShutdown.size();
		
		int DimmerTime = datacenter.getDimmerTime();
		
		double DimmerTimeRate = (double) DimmerTime /  datacenter.getTimesMayTriggerDimmer();
		...
		double revenue = datacenter.getDataCenterRevenueLoss();
		...
		if (outputInCsv) {
			...
			data.append(String.format("%.2f", revenue) + delimeter);
			...
			Log.printLine(String.format("Total Shutdown Time (Time Unit): %f", totalShutdownTime / 300));
			Log.printLine(String.format("Times of Dimmer Triggered: %d", DimmerTime));
			Log.printLine(String.format("Rate of Dimmer Triggered: %f", DimmerTimeRate));
...
//					stDevTimeBeforeVmMigration));
			Log.printLine(String.format(
					"Revenue loss of data center: %.2f", revenue));
			printNumberOfActiveHost(datacenter.getNumberOfActiveHostsMap());
...
//						.format("Execution time - total stDev: %.5f sec", executionTimeTotalStDev));
	 ...
	 * @author minxianx
	 */
	public static void printNumberOfActiveHost(LinkedHashMap<Double, Integer> lhm){
		Iterator<Entry<Double, Integer>> iterator = lhm.entrySet().iterator();
		while(iterator.hasNext()){
			Entry<Double, Integer> pairs = iterator.next();
			System.out.print(pairs.getKey() + " " );
		}

		System.out.println();
		
		Iterator<Entry<Double, Integer>> iterator1 = lhm.entrySet().iterator();
		while(iterator1.hasNext()){
			Entry<Double, Integer> pairs1 = iterator1.next();
			System.out.print(pairs1.getValue() + " " );
		}
	}
